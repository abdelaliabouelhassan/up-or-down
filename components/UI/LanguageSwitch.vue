<template>
  <div class="relative font-inter" id="lang">
    <div class="cursor-pointer flex items-center space-x-2" @click="openLang">
      <img class="w-[1.688rem] h-[1.688rem] rounded-full object-cover" v-if="!loading" :src="getLang.flag" alt="" />
      <span class=" text-white font-bold capitalize" v-if="showCode">{{getLang.code}}</span>
    ...
    <div v-if="show" class="absolute z-40 space-y-1.5 top-10 right-0 py-2 rounded-lg bg-white shadow">
      <div
        @click="setLang(lang)"
